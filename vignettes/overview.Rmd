...
rf = constant(x = -1000, y = 0, h = 8) # reference point

m = aem(k, top, base, n, hw1, hw2, rf) # create model

contours(m, xg, yg, col = 'dodgerblue', nlevels = 20) # plot head contours
...
When solving for unknown element parameters, a so-called reference point (or reference head) needs to be added to the model in order to have enough equations in the matrix formulation to solve all unknowns. This is a constant value which is added to all hydraulic head values. It can be created by the `constant()` function which needs an $x$ and $y$ coordinate and the reference head $h$. It is of importance to understand that this reference point is not an actual aquifer feature, but a mathematical necessity. Therefore, its value and location should be chosen is such a way that it does not influence the results. In general, the reference point should be never be placed inside the zone of interest (called the *"near-field"* in AEM jargon). It should be placed in the far-field and given a "neutral" head value as to not induce unrealistic gradients towards the near-field.

## Line-sink

Line-sinks are elements with a line geometry which can be used to simulate groundwater flow to and from surface water features such as streams or canals. The parameter of a line-sink is its *strength* $\sigma$, which is defined as the total flow to/from the line-sink per unit length of the line $[L^2/T]$ (positive going into the line-sink). The parameter is defined at the center of the line-sink.

Here, a single line-sinks is created using `linesink()`. In addition to the start- and endpoints of the line-sink $x_0-y_0 and x_1-y_1$, the strength parameters needs to be specified. Optionally, a width can also be specified, which for a strength-specified line-sink is only in particle tracking to determine when the particles have reached the line.

Note that the locations elements with line and point geometries (except the reference point) can be plotted using the `plot.aem()` method. Individual element locations can also be plotted by calling `plot.element()` on the respective element object.
...
As a general rule of thumb, line-sinks in the far-field should not have resistance or width values specified, so they act as constant-head boundaries which fix the head in the far-field. Line-sinks in the near-field on the other hand should have resistances and widths specified to give a more realistic presentation of the surface water features in that area.
...
Area-sinks are circular elements that can be added at the top or bottom of the model. They provide a constant flux and can be used to simulate areal recharge or leakage through the aquifer base. The circular geometry is sufficient as long as the disc covers the entire area of interest and the heads inside are fixed by e.g. head-specified line-sinks. When area-sinks are provided, the stream function no longer gives reasonable values, as explained above. Supplying flow by using an area-sink introduces vertical flow components, besides the ones that occur due to the curvature of the water-table when variably-saturated thickness is used in the `aem()` model.

The `areasink()` function requires the center coordinates of the circular area-sink, as well as the uniform flux value `N` $[L/T]$ (positive is into the aquifer, note that this differs from other elements) and a radius `R`. The value of `R` is not important as long as its large enough so that the disc covers the modeled region. By default, the area-sink is placed at the aquifer top, which can be overridden by setting `location = 'base'`.

To illustrate to effect of the area-sink providing areal recharge at the aquifer top, areal recharge is added to the example of the head-specified line-sinks above by use of an area-sink. Note the increased head values compared to the case without recharge.

```{r area-sink}
el$as = areasink(x = 0, y = 0, N = 0.001, R = 1000) # add to list of elements
...
A uniform-flow element may be created which models a specified background flow in the aquifer. In general, the regional gradient should be modeled by including head-specified surface water features in the model which will induce a certain background flow in the model. For certain applications however, it may be useful to specify this uniform background flow explicitly, e.g. when the regional system is included fully in the model. This can be done by using `uniformflow()`. It requires a transmissivity value which remains constant in the entire domain (i.e. has a constant saturated thickness), a gradient which is positive in the direction of flow and an angle in degrees counterclockwise from the positive x-axis:
...
# add stream lines
contours(m, xg, yg, variable = 'streamfunction', col = 'orange', nlevels = 20, add = TRUE)
```
...
The use of `add_element()` to set-up the model, which can be used to add elements to an initially empty model. After all the elements are added, the model needs to be solved with an explicit call to `solve.aem()`. This is useful for setting-up elements in a loop or pipeline.
...
base = -10      # aquifer base elevation, m
n = 0.2         # aquifer effective porosity, -
...
# add control points of line-sinks
for(i in m$elements) {
...
-   `satthick()` calculates the saturated thickness of the aquifer at given `x` and `y` locations. For confined aquifer, the saturated thickness always equals the aquifer thickness.
...
-   `flow_through_line()` calculates the amount of flow passing through a specified straight line by numerically integrating the requested flow variable ($Q$ or $q$).
